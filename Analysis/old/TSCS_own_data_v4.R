...
pop_sample = DMX_populist %>% 
  mutate(populist = ifelse(is.na(populist_pres) == F & populist_pres == 1, 1, populist)) %>% 
  group_by(country_name) %>% 
  select(country_name, year, pop_cat, populist) %>% 
  mutate(pop_lead = dplyr::lead(populist, 4)) %>% 
  mutate(pop_lag = dplyr::lag(populist, 4)) %>% 
  mutate(pop_tot = ifelse(pop_lead != 0 | pop_lag != 0, 1, 0)) %>% 
  filter(pop_tot == 1, year >= 1990) %>% 
  select(country_name, year)

TSCS_data_trans = DMX_populist %>% 
  right_join(pop_sample, by=c("country_name", "year")) %>% 
